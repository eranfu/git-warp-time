...
fn touch_if_time_mismatch(
    ...
    let commit_time = FileTime::from_unix_time(seconds, 0);
    ...
    if file_mtime > commit_time || (!ignore_older && file_mtime < commit_time) {
        ...
        if verbose {
            println!(
                "{} commit_time: {}, file_mtime: {}, Rewound the clock: {}",
                commit_id,
                commit_time.seconds(),
                file_mtime.seconds(),
                path
    ...
    if verbose {
        println!(
            "{} commit_time: {}, file_mtime: {}, No need to touch: {}",
            commit_id,
            commit_time.seconds(),
            file_mtime.seconds(),
            path
...
fn process_touchables(repo: &Repository, touchables: FileSet, opts: &Options) -> Result<FileSet> {
    ...
    println!("Touching files. num: {}.", touchables.len(),);
        ...
        touchable_oids.retain(|oid, touchable_path| {
            ...
            if affected {
                ...
                if let Ok(true) = touch_if_time_mismatch(
                    ...
                    touched.insert(touchable_path.to_path_buf());
                }
            }
            !affected
        });
